...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"🔔 welcome_new_member TRIGGERED! chat_id={update.effective_chat.id}, members={update.message.new_chat_members}")
...
async def debug_any_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """DEBUG: har group message ka detail log karta hai (temporary, testing ke liye)"""
    chat = update.effective_chat
    msg = update.message
    logger.info(
        f"🐛 DEBUG message received: chat_id={chat.id}, chat_type={chat.type}, "
        f"new_chat_members={msg.new_chat_members if msg else None}, "
        f"text={msg.text if msg else None}"
    )
...
def main():
    ...
    app.add_handler(MessageHandler(filters.ChatType.GROUPS | filters.ChatType.SUPERGROUP, debug_any_group_message), group=1)
